...
class ChoiceInlineFormset(forms.BaseInlineFormSet):
    def clean(self):
        ...
        try:
            ...
            raise forms.ValidationError(_('Tam olarak bir doğru seçime izin verilir.'))
...
class UserLoginForm(forms.Form):
    ...
    def clean(self, *args, **kwargs):
        ...
        if username and password:
            ...
            if not user:
                raise forms.ValidationError("Bu kullanıcı mevcut değil")
            if not user.check_password(password):
                raise forms.ValidationError("Yanlış parola")
            if not user.is_active:
                raise forms.ValidationError("Bu kullanıcı aktif değil")
